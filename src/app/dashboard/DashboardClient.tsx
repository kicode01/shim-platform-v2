    ...
    <div className="flex-1 flex flex-col max-w-7xl mx-auto w-full px-8 py-6 min-h-0 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6 shrink-0">
        ...
        <div className="flex items-center gap-4">
          <Link href="/dashboard/templates" className="flex items-center gap-2 bg-white border border-zinc-200 text-zinc-700 hover:bg-zinc-50 hover:text-zinc-700 px-4 py-2 rounded-md font-medium text-sm transition-colors shadow-sm">
            <Stamp size={16} />
            <span className="hidden sm:inline">Templates</span>
          </Link>
          <Link href="/dashboard/generate" className="flex items-center gap-2 bg-zinc-700 text-white hover:bg-zinc-700 px-4 py-2 rounded-md font-medium text-sm transition-colors shadow-sm">
            <Plus size={16} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6 shrink-0">
        {[
          ...
          <div key={idx} className="bg-white border border-zinc-200 rounded-xl p-5 flex flex-col justify-between min-h-[140px] relative group overflow-hidden">
            ...
            <div className="text-4xl font-bold text-zinc-700 mt-6 relative z-10">
              {stat.value}
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 flex-1 min-h-0">
        ...
        <div className="xl:col-span-2 bg-white border border-zinc-200 rounded-xl overflow-hidden shadow-sm flex flex-col h-full min-h-0">
          ...
          <div className="p-6 border-b border-zinc-200 flex justify-between items-center bg-white shrink-0">
            <h2 className="text-xl font-bold text-zinc-700">Recent Events</h2>
          ...
          <div className="flex-1 overflow-y-auto">
            {initialEvents.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full p-12 text-center">
                ...
                <h3 className="text-xl font-bold text-zinc-700 mb-2">No events found</h3>
              ...
              <div className="divide-y divide-zinc-100">
                {initialEvents.map((event) => (
                  ...
                  >
                    <div>
                      <h4 className="font-medium text-zinc-700 mb-1 group-hover:text-black transition-colors">{event.name}</h4>
                    ...
                    <div className="flex gap-6 text-sm text-zinc-600">
                      <div className="flex flex-col items-end">
                        <span className="font-semibold text-zinc-700">{event.attendeeCount}</span>
                        <span className="text-xs text-zinc-500">Attendees</span>
                      </div>
                      <div className="flex flex-col items-end">
                        <span className="font-semibold text-zinc-700">{event.credentialCount}</span>
        ...
        <div className="bg-white border border-zinc-200 rounded-xl shadow-sm flex flex-col h-full min-h-0">
          ...
          <div className="p-6 bg-zinc-50 border-t border-zinc-200 rounded-b-xl mt-auto text-center shrink-0">
            <div className="flex items-center justify-center gap-2 text-sm font-medium text-zinc-700 mb-1">
